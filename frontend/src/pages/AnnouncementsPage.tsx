  ...
  return <section className="detail-panel">
    ...
    {detail.loading ? <Loading /> : detail.error ? <ErrorState error={detail.error} retry={() => void detail.reload()} /> : item && <>
      <div className="detail-grid">
        <div><small>公告编号</small><b>{businessReference(item.business_no, "社区公告")}</b></div>
        <div><small>当前状态</small><StatusBadge value={item.status} /></div>
        <div><small>公告类型</small><b>{displayLabel(item.category, "社区通知")}</b></div>
        <div><small>发布范围</small><b>{Object.keys(item.audience_condition).length ? "指定楼栋住户" : "全社区"}</b></div>
      ...
      {versions && <section className="version-history" aria-label="版本历史">
        ...
        {!versions.length ? <Empty title="暂无版本记录" /> : versions.map((version) => <article className="version-card" key={version.version_no}>
          ...
          <small>{displayLabel(version.category, "社区通知")} · {version.source === "MANUAL" ? "人工编辑" : "采纳智能建议"}</small>
...
export function AnnouncementsPage() {
  ...
  return <><header className="page-heading"><div><span className="eyebrow">{canCreate ? "公告运营" : "社区信息"}</span><h1>社区公告</h1><p>{canCreate ? "按草稿、审核和发布状态推进公告。" : "重要通知、服务安排和社区消息集中在这里。"}</p></div>{canCreate && <button className="button primary" onClick={() => setFormOpen(true)}><Plus size={17} />新建草稿</button>}</header>{formOpen && <form className="form-card" onSubmit={(event) => void create(event)}><div className="form-card-title"><FileText /><div><h2>新建公告草稿</h2><p>适用楼栋留空时代表全社区，发布前可预览。</p></div></div><div className="form-grid"><label className="span-2">标题<input required maxLength={128} value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} /></label><label>分类<select value={draft.category} onChange={(event) => setDraft({ ...draft, category: event.target.value })}><option value="GENERAL">一般通知</option><option value="MAINTENANCE">维护通知</option><option value="SAFETY">安全通知</option><option value="EMERGENCY">紧急通知</option></select></label><label>适用楼栋<input placeholder="例如 1,2" value={draft.building_ids} onChange={(event) => setDraft({ ...draft, building_ids: event.target.value })} /></label><label className="span-2">正文<textarea required value={draft.body} onChange={(event) => setDraft({ ...draft, body: event.target.value })} /></label></div>{formError ? <ErrorState error={formError} /> : null}<div className="form-actions"><button type="button" className="button ghost" onClick={() => setFormOpen(false)}>取消</button><button className="button primary">保存草稿</button></div></form>}<div className={selectedId ? "master-detail" : ""}><section className="content-panel"><div className="panel-heading"><h2>{canCreate ? "公告工作队列" : "最新社区消息"}</h2>{canCreate && <span className="safe-note">发布与撤回仅限授权人员</span>}</div>{list.loading ? <Loading /> : list.error ? <ErrorState error={list.error} retry={() => void list.reload()} /> : !list.data?.items.length ? <Empty title="暂无可见公告" /> : <div className={canCreate ? "announcement-queue" : "announcement-feed"}>{list.data.items.map((item) => <button className="announcement-item entity-button" key={item.id} onClick={() => setSelectedId(item.id)}><div className="announcement-meta"><StatusBadge value={item.status} /><span>{displayLabel(item.category, "社区通知")}</span><time>{displayDate(item.published_at ?? item.created_at)}</time></div><h3>{item.title}</h3><p>{item.body}</p></button>)}</div>}</section>{selectedId && <AnnouncementDetail id={selectedId} onClose={() => setSelectedId(null)} onChanged={list.reload} />}</div></>;
}
