...
const actionFields: Record<string, ActionField[]> = {
  ...
  RECORD_PROGRESS: [{ name: "record_type", label: "记录类型", type: "select", options: [{ value: "APPOINTMENT", label: "预约上门" }, { value: "ARRIVAL", label: "已到现场" }, { value: "PROGRESS", label: "处理进展" }, { value: "BLOCKED", label: "暂时受阻" }] }, { name: "note", label: "进度说明", type: "textarea", required: true }],
...
function RepairDetail({ id, onClose, onChanged }: { id: string; onClose: () => void; onChanged: () => Promise<void> }) {
  ...
  return <section className="detail-panel"><div className="panel-heading"><div><span className="safe-note">{detail.data ? businessReference(detail.data.business_no, "当前工单") : "工单"}</span><h2>工单详情</h2></div><button className="icon-button" aria-label="关闭详情" onClick={onClose}><X /></button></div>{detail.loading ? <Loading /> : detail.error ? <ErrorState error={detail.error} retry={() => void detail.reload()} /> : detail.data && <><div className="repair-detail-summary"><div><span>当前位置</span><strong>{detail.data.location}</strong></div><StatusBadge value={detail.data.status} /><p>{detail.data.description}</p><small>{detail.data.assignee_id ? "已安排处理人员" : "等待安排处理人员"}</small></div><div className="action-row primary-actions">{detail.data.available_actions?.map((item) => <button className="button ghost" key={item} onClick={() => void openAction(item)}>{actionLabels[item] ?? "继续处理"}</button>)}</div><h3 className="section-title">处理进度</h3>{timeline.loading ? <Loading /> : timeline.error ? <ErrorState error={timeline.error} retry={() => void timeline.reload()} /> : !timeline.data?.length ? <Empty title="暂无进度记录" detail="工单开始流转后，关键进度会显示在这里。" /> : <div className="timeline">{timeline.data.map((item, index) => <div key={`${item.created_at}-${index}`}><span /><div><b>{actionLabels[item.action] ?? "工单进展"}</b><p>{displayLabel(item.from_status, "已创建")} → {displayLabel(item.to_status ?? detail.data?.status)}</p>{(item.reason || item.note) && <small>{item.reason ?? item.note}</small>}<time>{displayDate(item.created_at)}</time></div></div>)}</div>}</>}{action && <ActionDialog title={actionLabels[action] ?? "继续处理"} fields={fields} onClose={() => setAction(null)} onConfirm={execute} />}</section>;
}
