...
export function HomePage() {
  ...
  useEffect(() => {
    ...
    if (!existing) return;
    let active = true;
    void apiRequest<AgentReply>(`/api/agent/conversations/${existing}`)
      .then((status) => {
        if (active) setAction(status.pending_confirmation);
      })
      .catch(() => {
  ...
  async function sendAgent(text: string, slots?: Record<string, unknown>) {
    ...
    setPending(true);
    try {
      const reply = await apiRequest<AgentReply>(`/api/agent/conversations/${conversationId()}/messages`, {
        method: "POST",
  ...
  async function resolveConfirmation(confirmed: boolean) {
    if (!action) return;
    const reply = await apiRequest<AgentReply>(`/api/agent/conversations/${conversationId()}/confirmations`, {
      method: "POST",
