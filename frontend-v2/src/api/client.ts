export type ApiErrorKind =
  | "unauthenticated" | "forbidden" | "conflict" | "validation" | "rate-limited"
  | "unavailable" | "network" | "timeout" | "cancelled" | "missing-context"
...
export class ApiClient {
  constructor(
    ...
    private readonly onAuthenticatedUnauthorized: () => void | Promise<void> = () => undefined,
  ) {}

  async request<T>(descriptor: RequestDescriptor, path: string, options: RequestOptions = {}): Promise<T> {
    const context = this.requireContext(descriptor);
    const controller = new AbortController();
    let timedOut = false;
    const timeout = globalThis.setTimeout(() => { timedOut = true; controller.abort(); }, options.timeoutMs ?? 10_000);
  ...
  private requireContext(descriptor: RequestDescriptor): RequestContext {
    ...
    if (descriptor.authentication === "required" && !context.accessToken) {
      throw new ApiError("missing-context", 0, "AUTH_CONTEXT_REQUIRED", "缺少认证上下文。");
    }
    if (descriptor.house === "required" && !context.currentHouseId) {
      throw new ApiError("missing-context", 0, "HOUSE_CONTEXT_REQUIRED", "请先选择当前房屋。");
    }
    return context;
  }
  ...
  private headers(descriptor: RequestDescriptor, context: RequestContext, options: RequestOptions): Headers {
    const headers = new Headers(options.headers);
    headers.set("Accept", "application/json");
    headers.set("X-Request-ID", options.requestId ?? `web_v2_${crypto.randomUUID()}`);
    if (options.body !== undefined) headers.set("Content-Type", "application/json");
    if (descriptor.authentication === "required") headers.set("Authorization", `Bearer ${context.accessToken}`);
    if (descriptor.house === "required") headers.set("X-Current-House-ID", context.currentHouseId!);
    if (options.idempotencyKey) headers.set("Idempotency-Key", options.idempotencyKey);
    return headers;
  }

  private async parse<T>(descriptor: RequestDescriptor, response: Response): Promise<T> {
    if (response.status === 401 && descriptor.invalidateSessionOn401) {
      try { await this.onAuthenticatedUnauthorized(); } catch { /* Session invalidation remains best-effort and cannot mask the 401. */ }
    }
    let payload: unknown;
    try { payload = await response.json(); }
    catch { throw new ApiError("invalid-response", response.status, "INVALID_RESPONSE", "服务返回了无法识别的响应。"); }
    if (!response.ok) {
      throw this.httpError(response.status, payload);
    }
    if (descriptor.decoder === "direct") {
      if (typeof payload !== "object" || payload === null || Array.isArray(payload)) {
        throw new ApiError("invalid-response", response.status, "INVALID_RESPONSE", "响应不符合 direct 契约。");
      }
      return payload as T;
    }
  ...
  private parseEnvelope<T>(status: number, payload: unknown): T {
    if (typeof payload !== "object" || payload === null || typeof (payload as Partial<Envelope<T>>).success !== "boolean") {
      throw new ApiError("invalid-response", status, "INVALID_RESPONSE", "响应不符合 API Envelope 契约。");
    }
    const envelope = payload as Envelope<T>;
    if (envelope.success) return envelope.data as T;
    throw new ApiError("invalid-response", status, envelope.error?.code ?? "INVALID_ENVELOPE",
      envelope.error?.message ?? "响应缺少业务数据。", envelope.request_id, envelope.error?.details ?? null);
  }
  ...
  private httpError(status: number, payload: unknown): ApiError {
    ...
    return new ApiError(statusKinds[status] ?? "unknown", status, code, "请求未成功。", requestId);
  }
}
