...
from property_agent.platform.infrastructure.database import get_engine, get_session_factory
...
def _seed_users(session) -> None:
    ...
    session.add_all([
        ...
        UserModel(
            id=RESIDENT_A_SINGLE, community_id=COMMUNITY_A,
            username="zhangsan", display_name="张三",
            password_hash=password_hash, phone="138****6789",
            status="ACTIVE",
        ),
        UserModel(
            id=RESIDENT_A_MULTI, community_id=COMMUNITY_A,
            username="lisi", display_name="李四",
            password_hash=password_hash, phone="138****6790",
            status="ACTIVE",
        ),
        UserModel(
            id=RESIDENT_A_NOBIND, community_id=COMMUNITY_A,
            username="wangwu", display_name="王五",
            password_hash=password_hash, phone="138****6791",
            status="ACTIVE",
        ),
        # Community A — Staff
        UserModel(
            id=CS_A, community_id=COMMUNITY_A,
            username="customer_service", display_name="客服小刘",
            password_hash=password_hash, phone="139****8901",
            status="ACTIVE",
        ),
        UserModel(
            id=REPAIR_A, community_id=COMMUNITY_A,
            username="repair_worker", display_name="维修工老张",
            password_hash=password_hash, phone="139****8902",
            status="ACTIVE",
        ),
        UserModel(
            id=FINANCE_A, community_id=COMMUNITY_A,
            username="finance", display_name="财务小陈",
            password_hash=password_hash, phone="139****8903",
            status="ACTIVE",
        ),
        UserModel(
            id=SECURITY_A, community_id=COMMUNITY_A,
            username="security_guard", display_name="安保老李",
            password_hash=password_hash, phone="139****8904",
            status="ACTIVE",
        ),
        UserModel(
            id=DUTY_A, community_id=COMMUNITY_A,
            username="duty_officer", display_name="值班人员",
            password_hash=password_hash, phone="139****8905",
            status="ACTIVE",
        ),
        UserModel(
            id=MANAGER_A, community_id=COMMUNITY_A,
            username="manager", display_name="王经理",
            password_hash=password_hash, phone="137****0123",
            status="ACTIVE",
        ),
        UserModel(
            id=SYSADMIN_A, community_id=COMMUNITY_A,
            username="sysadmin", display_name="系统管理员",
            password_hash=password_hash, phone="137****0124",
            status="ACTIVE",
        ),
        # Community B — Resident
        UserModel(
            id=RESIDENT_B, community_id=COMMUNITY_B,
            username="qianqi", display_name="钱七",
            password_hash=password_hash, phone="138****6792",
            status="ACTIVE",
        ),
    ])
    print(f"  [+] 12 users seeded")
...
def _seed_bindings(session) -> None:
    ...
    session.add_all([
        ...
        UserHouseBindingModel(
            user_id=RESIDENT_A_SINGLE, house_id=HOUSE_A_101,
            status="ACTIVE",
        ),
        # Multi-house resident: 李四 -> 1-1-102 + 2-1-201
        UserHouseBindingModel(
            user_id=RESIDENT_A_MULTI, house_id=HOUSE_A_102,
            status="ACTIVE",
        ),
        UserHouseBindingModel(
            user_id=RESIDENT_A_MULTI, house_id=HOUSE_A_201,
            status="ACTIVE",
        ),
        # Expired binding (for testing)
        UserHouseBindingModel(
            user_id=RESIDENT_A_SINGLE, house_id=HOUSE_A_202,
            status="EXPIRED",
            valid_until=now - timedelta(days=30),
        ),
        # Community B resident
        UserHouseBindingModel(
            user_id=RESIDENT_B, house_id=HOUSE_B_101,
            status="ACTIVE",
        ),
    ])
    print(f"  [+] 5 bindings seeded")
