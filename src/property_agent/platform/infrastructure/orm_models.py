...
# ═══════════════════════════════════════════════════════════════
# 2. User — resident and staff accounts
# ═══════════════════════════════════════════════════════════════
...
class HouseModel(Base):
    ...
    __table_args__ = (
        UniqueConstraint("community_id", "building", "unit", "room_no", name="uq_houses_address"),
    ...
    status: Mapped[str] = mapped_column(
        String(16), nullable=False, default="ACTIVE", comment="状态: ACTIVE / INACTIVE / DECORATING"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=func.now(), comment="创建时间"
    )
...
class UserRoleModel(Base):
    ...
    scope: Mapped[str] = mapped_column(
        String(64), nullable=False, default="*", comment="授权范围: * 表示全社区, 或指定楼栋/区域"
    )
...
class UserHouseBindingModel(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(16), nullable=False, default="ACTIVE", comment="状态: ACTIVE / INACTIVE / EXPIRED"
    )
    valid_from: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=func.now(), comment="生效时间"
    )
...
class ConfirmationTokenModel(Base):
    ...
    token: Mapped[str] = mapped_column(String(256), nullable=False, unique=True, comment="令牌值")
    actor_id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), nullable=False, comment="操作人ID")
    action: Mapped[str] = mapped_column(String(64), nullable=False, comment="操作类型")
    parameter_hash: Mapped[str] = mapped_column(String(128), nullable=False, comment="参数哈希")
...
class IdempotencyRecordModel(Base):
    ...
    __table_args__ = (
        UniqueConstraint("actor_id", "operation", "key", name="uq_idempotency_actor_op_key"),
    ...
    key: Mapped[str] = mapped_column(String(128), nullable=False, comment="幂等键")
    request_hash: Mapped[str] = mapped_column(String(128), nullable=False, comment="请求体哈希")
    resource_id: Mapped[str | None] = mapped_column(String(64), comment="创建的资源ID")
...
class MessageRecordModel(Base):
    ...
    resource_id: Mapped[str] = mapped_column(String(64), nullable=False, comment="关联资源ID")
    ...
    idempotency_key: Mapped[str] = mapped_column(String(128), nullable=False, comment="消息去重键")
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=func.now(), comment="创建时间"
    )
    updated_at: Mapped[datetime] = mapped_column(
...
class AuditLogModel(Base):
    ...
    actor_id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), nullable=False, comment="操作人ID")
    community_id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), nullable=False, comment="社区ID")
    action: Mapped[str] = mapped_column(
        String(64),
        nullable=False,
        comment="操作: LOGIN_SUCCESS / LOGIN_FAILED / "
    ...
    resource_id: Mapped[str | None] = mapped_column(String(64), comment="资源ID")
    parameter_summary: Mapped[dict | None] = mapped_column(
        JSON().with_variant(JSONB(), "postgresql"), comment="参数摘要(脱敏)"
    )
    result: Mapped[str] = mapped_column(
        String(16), nullable=False, default="SUCCESS", comment="结果: SUCCESS / FAILURE / DENIED"
    )
...
class HandoverTicketModel(Base):
    ...
    requester_id: Mapped[UUID | None] = mapped_column(Uuid(as_uuid=True), comment="发起人ID")
    resource_type: Mapped[str | None] = mapped_column(
        String(32), comment="关联资源类型: WORK_ORDER / ANNOUNCEMENT / BILL / EVENT"
    )
    resource_id: Mapped[str | None] = mapped_column(String(64), comment="关联资源ID")
    request_id: Mapped[str | None] = mapped_column(String(64), comment="发起请求ID(链路追踪)")
    ...
    source: Mapped[str] = mapped_column(
        String(32),
        nullable=False,
        comment="来源: REPAIR / ANNOUNCEMENT / BILLING / INSPECTION / AGENT",
    )
    queue: Mapped[str] = mapped_column(
        String(32),
        nullable=False,
        comment="队列: CUSTOMER_SERVICE / REPAIR / SECURITY / MANAGEMENT",
    )
    summary: Mapped[str] = mapped_column(Text, nullable=False, comment="接管摘要")
    ...
    assignee_id: Mapped[UUID | None] = mapped_column(Uuid(as_uuid=True), comment="指派人ID")
...
class AttachmentModel(Base):
    ...
    content_type: Mapped[str] = mapped_column(String(128), nullable=False, comment="MIME 类型")
    ...
    storage_key: Mapped[str] = mapped_column(String(512), nullable=False, comment="对象存储键")
