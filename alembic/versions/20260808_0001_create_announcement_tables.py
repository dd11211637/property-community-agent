...
Re-parented onto the platform baseline (20260801_0001) when the announcement
module was integrated into the unified application: the announcement tables
reference platform tables (communities / users), so they must be created after
the platform schema exists.
"""
...
def upgrade() -> None:
    ...
    # Announcement audiences may be targeted by house type (住宅/商铺/办公/车位).
    # The platform baseline only stored building / unit / room_no, so the
    # dimension is added here rather than silently ignored by the resolver.
    op.add_column("houses", sa.Column("house_type", sa.String(length=32), nullable=True))
...
def downgrade() -> None:
    ...
    op.drop_column("houses", "house_type")
